...
/**
 * Fanning glow "threads" background, WebGL via ogl. Pauses its render loop
 * when off-screen (IntersectionObserver) or the tab is hidden (Page
 * Visibility API) - no continuous cost while not actually visible.
 */
export function WebThreads({
  ...
  useEffect(() => {
    ...
    // Listen on window, not the canvas - this background sits behind every
    // other page element (fixed inset-0 -z-10), so foreground content would
    // otherwise intercept the mousemove before it ever reaches the canvas.
